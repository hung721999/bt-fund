...
void on_connected(struct bt_conn *conn, uint8_t err)
{
	...
	if (info.le.phy->tx_phy == BT_CONN_LE_TX_POWER_PHY_1M)
	{
		LOG_INF("Current PHY: 1M");
	}
	else if (info.le.phy->tx_phy == BT_CONN_LE_TX_POWER_PHY_2M)
	{
		LOG_INF("Current PHY: 2M");
	}
	else if (info.le.phy->tx_phy == BT_CONN_LE_TX_POWER_PHY_CODED_S8)
	{
		LOG_INF("Current PHY: Long Range");
	}
